...
public class Configuration {
	...
	/** ADDED LINES FOR DB WRAPPER
	 /** mysql database wrapper*/
	public static final int MYSQL_WRAPPER = 7;

	/** sqlite database wrapper*/
	public static final int SQLite_WRAPPER = 8;

	/** postgre database wrapper*/
	public static final int PostgreSQL_WRAPPER = 9;

	/** Warpper model */
	public int dbWrapper=SQLite_WRAPPER;
	...
	private void parseConfigFile(String configFilename) throws Exception{
		...
		for(int j = 0; j < atts.getLength(); j++) {
            ...
            if(attName.compareToIgnoreCase("k") == 0) {
            	...
            	setMethod(method);
            } else if(attName.compareToIgnoreCase("wrapper") == 0) {
                String wrapper = atts.item(j).getNodeValue();
    ...
    /**
     * ADDED LINES: Sets the database type for anonymization
     * @param wrapper a db wrapper identifier
     */
    public void setDbWrapper(String wrapper) {
        if(wrapper.compareToIgnoreCase("mysql") == 0) {
            this.dbWrapper = MYSQL_WRAPPER;
        } else if(wrapper.compareToIgnoreCase("postgres") == 0) {
            this.dbWrapper = PostgreSQL_WRAPPER;
        } else if(wrapper.compareToIgnoreCase("sqlite") == 0) {
            this.dbWrapper = SQLite_WRAPPER;
        } else {
            System.out.println("WARNING: Unrecognized database wrapper, " +
                    "SQLite will be used for anonymization!!!");
            this.dbWrapper = SQLite_WRAPPER;
        }
    }
	...
	public void setOptions(String[] args) {
        ...
        if( (index = getOptionPos("-wrapper", args)) >= 0) {
            setDbWrapper(args[index]);
        }
	}
