...
public class AnonRecordTable {
    ...
    /** Database selection from configuration*/
    private DBSelector sel=new DBSelector();
	 ...
	 */
	public AnonRecordTable(Integer[] qidIndices, Integer[] sensIndices, String tableName) throws Exception {
		this.qidIndices = qidIndices;
		this.sensIndices = sensIndices;
		this.tableName = tableName;

        //get specified database in configuration file and check DB connectivity
        sqlwrapper=sel.getDB(); //check DB connectivity
...
//	}
	...
	private void createTable() {
        ...
        String select_SQL = "SELECT "+sel.getMastertable()+" FROM "+sel.getMasterdb()+" WHERE "+sel.getMastertable()+" = '" + tableName + "'";
		...
		for(int i = 0; i < qidIndices.length; i++) {
			createTable_SQL += "ATT_" + qidIndices[i] + " DOUBLE "+sel.getType()+", ";
		}
		for(int i = 0; i < sensIndices.length; i++) {
			createTable_SQL += "ATT_" + sensIndices[i] + " DOUBLE "+sel.getType()+", ";
		}
		//trim the last comma
